...
export function formatWarningMessage(warning: AgentWarning): string {
  return `${warning.impact} ${warning.why} To undo, ${warning.undo}.`;
}

/** Impact first, then the cause, then how to revert — flush left, dimmed. */
export function printAgentWarning(warning: AgentWarning): void {
  output.print('\n');
  output.warn(warning.impact);
  output.print(chalk.dim(`${warning.why}\n`));
