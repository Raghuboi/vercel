...
// The gateway-key keychain stays out of the picture (no keychain prompts, even
// on macOS dev machines) and the Claude Code login item is simulated so the
// darwin-only `security` probe never runs on CI or touches a real keychain.
const keychainState = vi.hoisted(() => ({ claudeLogin: false }));
...
vi.mock(
  ...
  async importOriginal => {
    ...
    return {
      ...
      keychainHasGenericPassword: (service: string) =>
        service === 'Claude Code-credentials'
          ? keychainState.claudeLogin
          : false,
    };
  }
);
...
beforeEach(() => {
  keychainState.claudeLogin = false;
...
describe('ai-gateway coding-agents connect — pre-existing logins', () => {
  describe('Claude Code (anthropic_login_conflict)', () => {
    ...
    it('non-interactive without naming the agent skips it with requires_consent', async () => {
      ...
      expect(skip.message).toContain('claude /logout');
    ...
    it('fails with a self-contained payload when every detected agent needs consent', async () => {
      ...
      expect(out.next[0].command).toBe(
        'vercel ai-gateway coding-agents connect --key <key> --agent claude-code'
      );
    ...
    it('a macOS Keychain credentials item counts as a login', async () => {
      keychainState.claudeLogin = true;
      expect(await claudeCode.warnings!({ home })).toEqual([
        expect.objectContaining({ code: 'anthropic_login_conflict' }),
      ]);
    });

    it('honors CLAUDE_CONFIG_DIR for login detection', async () => {
      const custom = join(home, 'custom-claude');
      loginClaude(custom);
      process.env.CLAUDE_CONFIG_DIR = custom;
      expect(await claudeCode.warnings!({ home })).toHaveLength(1);
